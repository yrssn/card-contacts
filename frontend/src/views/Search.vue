    ...
    <el-row :gutter="20">
      <el-col :xs="24" :md="12">
        ...
        <el-card style="margin-top:16px">
          ...
          <el-button :loading="testing" :disabled="!cfg.configured || !test.company" @click="runTest">检索并总结</el-button>
      ...
      <el-col :xs="24" :md="12">
        <el-card>
          ...
          <ol class="steps">
            ...
            <li>把 Key 粘贴到左侧保存，并用「测试一下」验证。</li>
            <li>之后每次识别名片：识别出公司名/官网 → 自动用 Tavily 搜索该公司 → 交给当前默认视觉模型总结，自动填好 <b>主营业务关键词</b>、<b>主营产品/服务类型</b>，并在页面显示公司概述；结果可手工修改后再录入。</li>
...
const cfg = reactive<SearchConfig>({ provider: 'tavily', api_key_masked: '', enabled: true, max_results: 5, configured: false })
const form = reactive({ api_key: '', enabled: true, max_results: 5 })
...
async function load() {
  ...
  Object.assign(form, { api_key: '', enabled: cfg.enabled, max_results: cfg.max_results })
}
async function save() {
  saving.value = true
  try {
